 ...
 * --------------------------------------------------------------------------------
 *
 *  Script Name: AW | WA Send Email To Approvers
 *
 *  Script Description:
 *  This is a custom workflow action script that will send an email to the approvers once a record
...
define((require) => {
  /**
   * Entry Point for custom workflow action script
   *
   * @type {Object} AppWrap_WA_SendEmailToApproverGroup.js
   *
   ...
   */
  const exports = {};
  const moduleName = "AppWrap_WA_SendEmailToApproverGroup.js";
  ...
  function onAction(context) {
    ...
    log.debug({
      title: logTitle,
      details: `Record ID: ${context.newRecord.id} | Record Type: ${context.newRecord.type} | Workflow ID: ${context.workflowId}`,
    });

    const sendEmailToApproverGroup = require("./modules/AppWrap_SendEmailToApproverGroup");

    const sendEmailToApproverGroupClass =
      new sendEmailToApproverGroup.SendEmailToApproversGroup(context.newRecord);

    const result = sendEmailToApproverGroupClass.sendEmailToApproverGroup();
    log.debug({
      title: logTitle,
